...
class DataBaseUtils {
  ...
  static CollectionReference<ServiceProvider> getServiceProvideCollection() {
    return FirebaseFirestore.instance
        .collection(ServiceProvider.COLLECTION_NAME)
  ...
  static Future<void> AddServiceProvideToFirestore(
      ServiceProvider serviceProvide) {
    return getServiceProvideCollection()
        .doc(serviceProvide.id)
        .set(serviceProvide);
  }

  static Future<ServiceProvider?> readServiceProviderFromFirestore(
      String id) async {
  ...
  static Future<QuerySnapshot<Updates>> getClinicUpdates(String type) {
    return getCategoriesCollection().where('catId', isEqualTo: type).get();
  }
}
